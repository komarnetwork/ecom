...
class HomeController extends Controller
{
    ...
    {
        $detailedProduct  = Product::with('product_translations', 'product_translations', 'brand', 'user', 'reviews', 'wishlists', 'stocks', 'taxes')->where('slug', $slug)->where('approved', 1)->first();
    ...
    public function show_product_upload_form(Request $request)
    {
        ...
        $categories = Category::with('childrenCategories', 'category_translations', 'categories')->where('parent_id', 0)
            ->where('digital', 0)
            ->with('childrenCategories')
            ->get();
        return view('frontend.user.seller.product_upload', compact('categories'));
    ...
    {
        $product = Product::with('product_translations', 'category', 'brand', 'user', 'stocks', 'taxes')->findOrFail($id);
        $lang = $request->lang;
    ...
    {
        $search = null;
        $products = Product::with('product_translations', 'category', 'user')->where('user_id', Auth::user()->id)->where('digital', 0)->orderBy('created_at', 'desc');
    ...
    {
        $category = Category::with('category_translations', 'products', 'categories', 'childrenCategories', 'parentCategory')->where('slug', $category_slug)->first();
        if ($category != null) {
    ...
    {
        $product = Product::with('product_translations', 'stocks', 'taxes')->find($request->id);
        $str = '';
        $quantity = 0;
        $tax = 0;
        $max_limit = 0;
    ...
    {
        $category = Category::with('category_translations', 'products', 'categories')->findOrFail($request->id);
